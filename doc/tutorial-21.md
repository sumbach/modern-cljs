...
deeply refactoring both the directory layout and the `project.clj`
configuration of his very interesting and promising lib with the
intention of preparing it to be unit tested using
[clojurescript.test][4].

## Preamble

If you did not type by yourself all the changes we made to the cloned
`Enfocus` during the [previous tutorial][1], I suggest you to start
working by first cloning the following repo which incorporates the above
...
The first instinct would be to move on by starting to implement a few
unit tests for the `enfocus` lib based on the [clojurescript.test][4]
unit testing lib. But we are going to postpone this topic to the next
tutorial for a couple of reasons:

* first, to be able to define any unit tests for the `Enfocus`
  lib, we should at least know its basic use cases which we still have
  to learn about;
* second, we first want to make a few `project.clj` settings
...
directory layout and `project.clj` of `Enfocus` by:

* preparing the lib to be correctly packaged in a `jar`;
* instrumenting the lib with [piggieback][5];
* extending the application of the separation of concerns principle
...
with the `:crossovers` option of the `cljsbuild` plugin, we replaced
...
cljsbuild clean, cljsbuild once`.

Following is the list of the supported `lein` builtin tasks when
...
know about them. Let's now try the `lein jar` task by first
looking at the associated help:
...
Good. The `lein jar` task seems to be exactly what we need to package
...
JS files generated by `cljsbuild`) to be able to use `Enfocus`,
because those resources are used for testing purposes only.
...
> ATTENTION NOTE
>
> In [Tutorial 1 - The Basics][14] we already advised you that the
> `cljsbuild` plugin does not add to the Leiningen `:source-paths`
...
option the `cljsbuild`, all we have to do is add to the
...
So, roll back by removing the `:jar` option from the `:whitespace`
build.
...
At the beginning of the [previous tutorial][1] we looked at the
quasi standard directory layout of a mixed CLJ/CLJS project and
...
developer to use the lib itself has to reside in the `resources/`
directory. On the other hand, any static resource needed during the
developing/testing phase of a lib should to be saved in the
...
purposes only, not for releasing the lib to third parties.

Let's rename the `resources/` directory to `dev-resources/` and
modify references to it in `project.clj`.
...
As you can see, each `:output-to` and `:test-commands` setting is now
...
Great. We now have a jar package that includes exactly what is needed
...
[piggieback][5] lib introduced in
[Tutorial 19 - Livin' On the Edge][7] by adding
it to the `:dev` profile. We're also going to exploit this opportunity
to move few other things to the `:dev` profile as well.
...
the `piggieback` instrumentation which requires the
`:repl-options` and the `:injections` settings as well.

Next we moved more stuff to the `:dev` profile:

* `:test-paths` because it will be used for testing purposes only;
* `clojurescript.test` plugin, because it will be used for testing purposes only;
* the three builds (i.e., `:whitespace`, `:simple` and `:advanced`),
  because they will be used for testing purposes only;
* the three `:test-commands` (i.e., `"whitespace"`, `"simple"` and
  `"advanced"`) becasue they are used for testing purposes only.

Finally we added a `:deploy` build in the `:builds` section outside of the
`:dev` profile. This build is only to force the `.jar` output to include
the files contained in its `:source-paths` pathnames. Note that to
speed up this useless compilation we declared its compiler
`:optimizations` to be `:none`.

> NOTE 2: the `org.clojars.magomimmo` group-id is used to deploy the
> `Enfocus` artifact to clojars by respecting its guidelines. This
> artifact is said to be non-canonical and is a fork of the
> canonical one.


> NOTE 3: Due to `piggieback` requirements, we needed to update
> the `:min-lein-version` setting to `"2.2.0"`.


> NOTE 4: For a more detailed explanation of the `piggieback`
> instrumentation and the addition of the `:profiles` setting, see
> [Tutorial 18 - Housekeeping][8].

### Light the fire

We can now try to run everything from scratch to verify if the
...
engine as we explained in [Tutorial 18 - Housekeeping][8].
...
after all the changes we've made, the `Enfocus` lib is still working as
expected.
...
We did a lot of working by refactoring the directory layout of
...
revised `Enfocus` lib without incurring in any trouble.
...
Now reorganize the directory layout by following the quasi standard
directory layout we introduced at the beginning of the
[previous tutorial][1].
...
Let's now go back to `Enfocus`, because it's now time for
instrumenting `Enfocus` for REPLing with the browser.

## Add and configure `ring` and `compojure`

First we need to install and run an HTTP server as we did in
[Tutorial 3 - Ring and Compojure][10]. This time, instead of using
...
Here the only news, as compared to what we did in
[Tutorial 3 - Ring and Compojure][10], is the definition of the
`run` function which will be used to programmatically start the HTTP
server from the REPL on port `3000`.

> NOTE 5: The `:join? false` setting is used to return control to
> the REPL after the server has started. The `defonce` macro is used to
> bind the `server` symbol to the HTTP server on the first invocation of
> `run`; subsequent invocations will return the same HTTP server value.

As you can see, we defined the HTTP root "/" route to redirect to the
`/index.html` URL. This is the page that we're now going to create in
...
the connection with the browser in the same way we did in
...
in development and testing scenarios only. We also created
`connect.cljs` in the `src/brepl/enfocus/` directory path to keep
it separated from the rest of the codebase.

As a next step we need to instruct `cljsbuild` to include
...
> include this directory in the JVM classpath for the `:dev` profile.
> If we don't do
> this the `ring` server will not be able to locate the `public/index.html` page.
...
Great. The project is still compiling as expected and you can
verify that the `whitespace.js` script includes the contents of the
`src/brepl/` directory path too.

Now let's see if the unit tests are still working.
...
`"advanced"`) are still working. As you remember, we defined a single
failing unit test to remind us that we still have to define unit
tests for `Enfocus`.

### Enfocus bREPLing

As a final step, let's run the HTTP server and the bREPL from the
...
To create the bREPL connection just call the `browser-repl` function we
...
Then visit the [localhost:3000][12] URL. Wait a moment to allow the
...
```bash
git add dev-resources src/brepl test/clj
git commit -am "Instrumented"
```

Stay tuned for the next tutorial.

# Next Step - [Tutorial 22 - Learning by Contributing (Part 3)][13]
...
to implement some unit tests which follow my personal strategy when
unit testing. In doing that we'll discover a few bugs in the codebase
and we even correct them by first interacting with the REPL.
